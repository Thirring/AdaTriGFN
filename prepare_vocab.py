...
def parse_args():
    ...
    parser.add_argument('--data_dir', default='data_ann_none', help='data directory.')
    parser.add_argument('--vocab_dir', default='data_ann_none', help='Output vocab directory.')
    parser.add_argument('--lower', default=False, help='If specified, lowercase all words.')
    args = parser.parse_args()
    return args

def get_vocab(train_data_list, test_data_list, vocab_dir):


    # output files
    # token
    vocab_tok_file = vocab_dir / 'vocab_tok.vocab'
    # position
    vocab_post_file = vocab_dir / 'vocab_post.vocab'
    # deprel
    vocab_deprel_file = vocab_dir / 'vocab_deprel.vocab'
    # postag
    vocab_postag_file = vocab_dir / 'vocab_postag.vocab'
    # syn_post
    vocab_synpost_file = vocab_dir / 'vocab_synpost.vocab'

    # load files
    print("loading files...")
    train_tokens, train_deprel, train_postag, train_postag_ca, train_max_len = load_tokens(train_data_list)
    test_tokens, test_deprel, test_postag, test_postag_ca, test_max_len = load_tokens(test_data_list)
    ...
    deprel_counter = Counter(train_deprel  + test_deprel)
    postag_counter = Counter(train_postag  + test_postag)
    postag_ca_counter = Counter(train_postag_ca + test_postag_ca)
    ...
    return post_vocab, deprel_vocab, postag_vocab, syn_post_vocab
    ...
    for d in data:
        ...
        for i in range(n):
            for j in range(n):
                ...
                tup = tuple([d['postag'][i], d['postag'][j]])
